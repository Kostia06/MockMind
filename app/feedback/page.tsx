...
export default function FeedbackPage() {
  ...
  return (
    <div className="min-h-screen bg-black p-3 sm:p-4 lg:p-6 relative overflow-y-auto">
      {/* Background */}
      ...
      <div className="max-w-6xl mx-auto relative z-10 space-y-4 sm:space-y-6 py-4">
        {/* Header */}
        <motion.div
        ...
        >
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white mb-2 sm:mb-3">
            Your Interview
            <span className="block bg-gradient-to-r from-emerald-400 via-green-300 to-teal-400 bg-clip-text text-transparent">
              Performance Report
            </span>
          </h1>
          <p className="text-gray-400 text-sm sm:text-base">Detailed analysis and actionable feedback</p>
        </motion.div>
        ...
        >
          ...
          <div className="relative bg-zinc-900/80 backdrop-blur-2xl rounded-xl sm:rounded-2xl shadow-2xl p-5 sm:p-6 lg:p-8 border border-white/10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                ...
                >
                  ...
                  <svg className="w-36 h-36 sm:w-48 sm:h-48 transform -rotate-90">
                    ...
                    <motion.circle
                      ...
                      style={{ strokeDasharray: 2 * Math.PI * 64 }}
                      className="sm:hidden"
                    />
                    <motion.circle
                      cx="96"
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <div className={`text-4xl sm:text-6xl font-black bg-gradient-to-r ${getScoreColor(feedback.overallScore)} bg-clip-text text-transparent`}>
                        {feedback.overallScore}
                      </div>
                      <div className="text-gray-400 text-xs sm:text-sm font-medium">out of 10</div>
                    </div>
                  </div>
                </motion.div>
              </div>
              ...
              <div className="flex flex-col justify-center">
                ...
                <div className={`p-4 sm:p-6 rounded-2xl border-2 ${getReadinessStyle(feedback.interviewReadiness)}`}>
        ...
        >
          ...
          <div className="relative bg-zinc-900/80 backdrop-blur-2xl rounded-2xl shadow-2xl p-4 sm:p-6 border border-white/10">
            ...
            <div className="bg-black/40 rounded-xl p-4 sm:p-5 border border-white/10">
              <div className="flex flex-col sm:flex-row items-start sm:items-baseline gap-2 sm:gap-3 mb-3">
                <span className="text-xs sm:text-sm font-semibold text-gray-400">Filler Words Detected:</span>
        ...
        <motion.div
          ...
          className="bg-zinc-900/80 backdrop-blur-2xl rounded-xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 border border-white/10"
        >
          ...
          <div className="space-y-2 sm:space-y-3">
            {feedback.suggestions.map((suggestion, idx) => (
              <motion.div
                ...
                className="group/item flex gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-black/40 border border-white/10 hover:border-purple-400/30 hover:bg-white/5 transition-all"
              >
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center flex-shrink-0 text-white font-bold text-xs sm:text-sm group-hover/item:scale-110 transition-transform">
                  {idx + 1}
                </div>
                <p className="text-gray-300 text-xs sm:text-sm leading-relaxed">{suggestion}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        {feedback.answerQualityByQuestion.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="space-y-3 sm:space-y-4">
              {feedback.answerQualityByQuestion.map((item, idx) => (
                <motion.div
                  ...
                  className="p-4 sm:p-5 rounded-xl bg-black/40 border border-white/10 hover:border-indigo-400/30 transition-all"
                >
                  <div className="flex justify-between items-center mb-3 flex-wrap gap-2 sm:gap-3">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <span className="text-xs sm:text-sm font-bold bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">
                        {item.quality}/10
                      </span>
                    </div>
                  </div>
                  <p className="text-gray-400 text-xs sm:text-sm leading-relaxed">{item.feedback}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
        ...
        >
          <button
            onClick={handleBackHome}
            className="group relative flex-1 px-6 sm:px-8 py-3 sm:py-4 rounded-xl overflow-hidden transition-all"
          >
          ...
          >
            ...
            <span className="relative text-white font-semibold text-base sm:text-lg flex items-center justify-center gap-2">
